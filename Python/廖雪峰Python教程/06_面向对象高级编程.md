...
这是因为==直接显示变量调用的不是`__str__()`，而是`__repr__()`，两者的区别是`__str__()`返回用户看到的字符串，而`__repr__()`返回程序开发者看到的字符串，也就是说，`__repr__()`是为调试服务的。==

解决办法是再定义一个`__repr__()`。但是==通常`__str__()`和`__repr__()`代码都是一样的==，所以，有个偷懒的写法：
